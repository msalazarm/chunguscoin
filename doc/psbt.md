# PSBT Howto for Chunguscoin Core

Since Chunguscoin Core 0.17, an RPC interface exists for Partially Signed Chunguscoin
...
PSBT is an interchange format for Chunguscoin transactions that are not fully signed
...
#### Multisig with multiple Chunguscoin Core instances

Alice, Bob, and Carol want to create a 2-of-3 multisig address. They're all using
Chunguscoin Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `chunguscoin-cli` is used.
