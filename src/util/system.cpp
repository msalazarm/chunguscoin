...
const char * const BITCOIN_CONF_FILENAME = "chunguscoin.conf";
...
#else
    const char* pszModule = "chunguscoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Chunguscoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Chunguscoin";
#else
    // Unix
    return pathRet / ".chunguscoin";
#endif
#endif
}
