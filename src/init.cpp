 ...
 */
static const char* BITCOIN_PID_FILENAME = "chunguscoind.pid";
...
void Shutdown(InitInterfaces& interfaces)
{
    ...
    RenameThread("chunguscoin-shutoff");
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/chunguscoin-project/chunguscoin>";
    const std::string URL_WEBSITE = "<https://chunguscoin.org>";
...
static void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("chunguscoin-loadblk");
...
bool AppInitMain(InitInterfaces& interfaces)
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if chunguscoin is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if chunguscoin is started while in a temporary directory.\n",
